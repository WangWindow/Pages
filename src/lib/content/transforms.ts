...
/**
 * Get cover URL from cover field
 * Handles both ImageMetadata (from image() schema) and string URLs
 */
export function getCoverUrl(cover: ImageMetadata | string | undefined): string | undefined {
  if (!cover) return undefined;

  // ImageMetadata from Astro's image() has a src property
...
const fieldExtractors: { [K in keyof PostFieldMap]: (post: BlogPost) => PostFieldMap[K] } = {
  ...
  date: (p) => p.data.date,
  cover: (p) => p.data?.cover, // Now directly returns ImageMetadata or string
  tags: (p) => p.data?.tags,
